...
function search_google_unqprfx_plugin_meta( $links, $file ) { // add 'Support' and 'Donate' links to plugin meta row
	if ( strpos( $file, 'search-google.php' ) !== false ) {
		$links = array_merge( $links, array( '<a href="http://web-profile.net/wordpress/plugins/search-google/">' . __('Support', 'search-google') . '</a>' ) );
		$links = array_merge( $links, array( '<a href="http://web-profile.net/donate/">' . __('Donate', 'search-google') . '</a>' ) );
	}
	return $links;
}
